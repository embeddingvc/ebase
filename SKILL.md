...
Once it finishes, open a **new** Claude Code session in this directory and run `/ebase-setup` there — a session already running (e.g. the one that ran the install command) won't see the newly registered MCP server or skill until it's restarted.
